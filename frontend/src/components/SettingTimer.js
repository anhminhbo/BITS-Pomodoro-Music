...
const SettingTimer = () => {
  ...
  const handleCloseAndOpen = async () => {
    ...
    // Handle when open the settings again the state is still set
    await getSettings();
  };

  // Handle save properties in Setting
  const handleSave = () => {
    const focusLength = document.getElementById("setting-focus-length-min")
      .value
      ? document.getElementById("setting-focus-length-min").value
      : 0;
    const breakLength = document.getElementById("setting-break-length-min")
      .value
        ...
        <>
            ...
            <div className="timer-label">
              <div
                ...
                style={{
                  color: isFocused ? "#04AA6D" : "#5c8a07",
                  fontWeight: "bold",
                }}
              >
                {isFocused ? "Focus Mode" : "Break Mode"}
              </div>
              <div id="timer-time">
                {formatTime(TimerMin)}:{formatTime(TimerSec)}
              </div>
            </div>
          </div>
          <div
            className="timer-btn"
            style={{ height: isFocused ? "156px" : "104px" }}
          >
            {isFocused ? (
              <button
