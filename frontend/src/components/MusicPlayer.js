...
const MusicPlayer = () => {
    ...
    const getData = (link) => {
        ...
        .then(response => {
            ...
            setVideo(response.items[0].snippet);
        })
        .catch();
    }
    ...
    return (
        <div>
            {/* Input field */}
            <input type="text" onChange={(e) => youtubeURL.current = e.target.value}/>
            <input type="button" value="Add" onClick={(e) => getData(youtubeURL.current)}/>
