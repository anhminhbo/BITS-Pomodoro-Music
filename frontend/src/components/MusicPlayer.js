...
const MusicPlayer = () => {
  ...
  //   Debounce to handle multiple clicks at an 0.2s interval
  const handleDebounceSetIndex = debounce((value) => {
    setCurIndex(value);
  }, 200);

  const debounceClick = useCallback(
    (value) => handleDebounceSetIndex(value),
    []
  );
